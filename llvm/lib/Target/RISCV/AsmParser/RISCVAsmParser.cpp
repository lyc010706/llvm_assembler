...
//#include "llvm/MC/MCParser/MCTargetAsmParser.h"  //in dce
...
class RISCVAsmParser : public MCTargetAsmParser {
  ...
  //newly add
  bool MatchInstruction(SMLoc IDLoc, unsigned &Opcode,
                               OperandVector &Operands, MCFunctionModule *MCFM,unsigned int &curbasicblockNumcur,unsigned int &maxbasicblockNummax,
                               uint64_t &ErrorInfo,
                               bool MatchingInlineAsm) override;
  
  bool deadcodeElimination(MCFunctionModule *MCFM);
  
  bool validateBranchIns(MCInst Inst);
  ...
  //newly add
  bool emitInstruction(MCFunctionModule *MCFM,MCStreamer &Out) override;
  bool processBranchInstruction(MCInst &Inst, SMLoc IDLoc, unsigned int &curbasicblockNumcur,unsigned int &maxbasicblockNummax,
...
// bool RISCVAsmParser::deadcodeElimination(MCFunctionModule *MCFM)
// {
//    RISCVMcdce dce(MCFM);
//    dce.
// }
//newly add
bool RISCVAsmParser::MatchInstruction(SMLoc IDLoc, unsigned &Opcode,
                                             OperandVector &Operands,
                                             MCFunctionModule *MCFM,unsigned int &curbasicblockNumcur,unsigned int &maxbasicblockNummax,
                                             uint64_t &ErrorInfo,
                                             bool MatchingInlineAsm) 
{
  ...
  {
    MCFM->islastJ=0;
    bool flag=processBranchInstruction(Inst, IDLoc, curbasicblockNumcur, maxbasicblockNummax,MCFM);
    MCInstInfo *temp=new MCInstInfo(Inst,IDLoc);
    MCFM->insertInstoBasicblock(temp,curbasicblockNumcur);
    ...
    {
      maxbasicblockNummax++;
      MCFM->insertBasicBlock(maxbasicblockNummax,curbasicblockNumcur);
      curbasicblockNumcur=maxbasicblockNummax;
    }
    return false;
  }
  }             
...
bool RISCVAsmParser::processBranchInstruction(MCInst &Inst, SMLoc IDLoc, unsigned int &curbasicblockNumcur,unsigned int &maxbasicblockNummax,
                          MCFunctionModule *MCFM)
{
  switch (Inst.getOpcode()) 
  {
    ...
    {
      MCFM->islastJ=1;
      if(!(MCFM->blockNum.count(((MCSymbolRefExpr *)((Inst.getOperand(1).getExpr())))->getSymbol().getName())))
      {
         maxbasicblockNummax++;
         MCFM->blockNum[((MCSymbolRefExpr *)((Inst.getOperand(1).getExpr())))->getSymbol().getName()]=maxbasicblockNummax;
         MCFM->insertBasicBlock(maxbasicblockNummax,curbasicblockNumcur);
      }
      else
      {
        MCFM->insertBasicBlock(MCFM->blockNum[((MCSymbolRefExpr *)((Inst.getOperand(1).getExpr())))->getSymbol().getName()],curbasicblockNumcur);
      }
      return false;
    }
    case RISCV::BEQ:
    ...
    case RISCV::BGEU:
    {
      if(!(MCFM->blockNum.count(((MCSymbolRefExpr *)((Inst.getOperand(2).getExpr())))->getSymbol().getName())))
      {
         maxbasicblockNummax++;
         MCFM->blockNum[((MCSymbolRefExpr *)((Inst.getOperand(2).getExpr())))->getSymbol().getName()]=maxbasicblockNummax;
         MCFM->insertBasicBlock(maxbasicblockNummax,curbasicblockNumcur);
      }
      else
      {
        MCFM->insertBasicBlock(MCFM->blockNum[((MCSymbolRefExpr *)((Inst.getOperand(2).getExpr())))->getSymbol().getName()],curbasicblockNumcur);
      }
      return true;
    }
...
bool RISCVAsmParser::emitInstruction(MCFunctionModule *MCFM,MCStreamer &Out)
{
   RISCVMcdce dce(MCFM);
   dce.dce();
    ...
    while(tempnode!=NULL)
    {
      if(tempnode->instInfo->isSymbol==0)
      {
        emitToStreamer(Out, tempnode->instInfo->inst);
      }
      else
      {
        Out.emitLabel(tempnode->instInfo->Sym, tempnode->instInfo->IDLoc);
      }
