...
private:
  //newly add
  MCList<MCFunctionModule*> MCFMList;
  MCFunctionModule* currMCFuntion;
  int MCFuntionNum;
  //MCFunctionModule &MCFM;
  unsigned int curbasicblockNum;
  unsigned int maxblockNum;
  //unordered_map<StringRef,int> blockNum;
...
public:
  ...
  bool insertMCFunction() override;//newly add
...
AsmParser::AsmParser(SourceMgr &SM, MCContext &Ctx, MCStreamer &Out,
  ...
  MCFuntionNum=0;
  // MCFunctionModule *temp=new MCFunctionModule(0);
  // temp->insertfirstBasicBlock(0);
  // MCFMList.insertBack(temp);
  // curbasicblockNum=0;
  // maxblockNum=0;
}
...
bool AsmParser::Run(bool NoInitialTextSection, bool NoFinalize) {
  ...
  //getTargetParser().emitInstruction(MCFMList.firstNode->instInfo,Out);
...
bool AsmParser::parseStatement(ParseStatementInfo &Info,
  ...
  if (Lexer.is(AsmToken::Colon) && getTargetParser().isLabel(ID)) {
    ...
    if (LocalLabelVal == -1) {
      ...
      if(IDVal.starts_with(".Lfunc_end"))
      {
        getTargetParser().emitInstruction(currMCFuntion,Out);
      }

      if(IDVal.starts_with(".LBB"))
      {
        if(currMCFuntion->islastJ!=1)
        {
            if(!(currMCFuntion->blockNum.count(IDVal)))
            {
              maxblockNum++;
              currMCFuntion->blockNum[IDVal]=maxblockNum;
              currMCFuntion->insertBasicBlock(maxblockNum,curbasicblockNum);
              curbasicblockNum=maxblockNum;
            }
            else
            {
              unsigned int tempblockNum=currMCFuntion->blockNum[IDVal];
              currMCFuntion->insertBasicBlock(tempblockNum,curbasicblockNum);
              curbasicblockNum=tempblockNum;
            }
        }
        else
        {
            if(!(currMCFuntion->blockNum.count(IDVal)))
            {
              maxblockNum++;
              currMCFuntion->blockNum[IDVal]=maxblockNum;
              currMCFuntion->inserttempbb(maxblockNum);
              curbasicblockNum=maxblockNum;
            }
            else
            {
        ...
        {
          MCInstInfo  *MCInsttemp=new MCInstInfo(Sym,IDLoc);
          currMCFuntion->insertSym(MCInsttemp);
        }
...
bool AsmParser::insertMCFunction()
{
  MCFunctionModule *temp=new MCFunctionModule(MCFuntionNum);
  temp->insertfirstBasicBlock(0);
  MCFMList.insertBack(temp);
  curbasicblockNum=0;
  maxblockNum=0;
...
bool AsmParser::parseAndMatchAndEmitTargetInstruction(ParseStatementInfo &Info,
  ...
  {
    uint64_t ErrorInfo;
    if (getTargetParser().MatchInstruction(
            IDLoc, Info.Opcode, Info.ParsedOperands, currMCFuntion,curbasicblockNum, maxblockNum,ErrorInfo,
            getTargetParser().isParsingMSInlineAsm()))
      return true;
  }
  return false;
