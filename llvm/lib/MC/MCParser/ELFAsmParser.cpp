...
bool ELFAsmParser::ParseDirectiveType(StringRef, SMLoc) {
  ...
  if(Type.starts_with("functio"))
  {
    const MCExpr *Subsection = nullptr;
      std::string s1=".text.";
      std::string combined = s1 +  Name.str();
      StringRef Section=combined;
         getStreamer().switchSection(getContext().getELFSection(Section, ELF::SHT_PROGBITS, ELF::SHF_EXECINSTR |ELF::SHF_ALLOC),
                         Subsection);
        getParser().insertMCFunction();
  }
